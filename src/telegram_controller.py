...
class TelegramController:
    def __init__(self, token, chat_id, database, gerador_relatorios):
        ...
        try:
            ...
            self.logger.error(
                f"Erro ao verificar credenciais do Telegram: {e}")
            raise

    def _escapar_markdown(self, texto):
        # Apenas caracteres básicos do Markdown
        caracteres = ['_', '*', '`', '[']
        for c in caracteres:
    ...
    def enviar_mensagem(self, mensagem, keyboard=None):
        try:
            ...
            response.raise_for_status()
            self.logger.info(
                f"Mensagem enviada com sucesso: {mensagem[:100]}...")
    ...
    def processar_mensagem(self, mensagem):
        try:
            ...
            acoes_botoes = {
                "🕒 Registrar Ponto": lambda: self.registrar_ponto_manual([]),
                "📊 Status": lambda: self.mostrar_status_detalhado([]),
                "⏸️ Pausar Sistema": lambda: self.processar_botao("⏸️ Pausar Sistema"),
                "▶️ Retomar Sistema": lambda: self.processar_botao("▶️ Retomar Sistema"),
                "📄 Relatório Mensal": lambda: self.enviar_relatorio([]),
                "📋 Relatório Anual": lambda: self.gerar_relatorio_anual([]),
                "⏰ Horas Trabalhadas": lambda: self.mostrar_horas([]),
                "❌ Falhas": lambda: self.mostrar_falhas([]),
                "⚙️ Configurações": lambda: self.mostrar_configuracoes([]),
                "❓ Ajuda": lambda: self.mostrar_ajuda([]),
                "❌ Encerrar": lambda: self.processar_botao("❌ Encerrar")
            }
        ...
        try:
            ...
            if registros_hoje:
                for reg in registros_hoje:
                    try:
                        dt = datetime.fromisoformat(reg[1])
                        msg += f"• {dt.strftime('%H:%M')} - {reg[2]} ({reg[3]})\n"
                    except ValueError:
                        # Caso a data venha em outro formato
            ...
            if horas_hoje:
                ...
                try:
                    ...
                    self.logger.error(f"Erro ao processar horas: {e}")
                    msg += "Erro ao processar horas trabalhadas\n"
            else:
            ...
            try:
                ...
                self.logger.error(f"Erro ao calcular próximos horários: {e}")
                msg += "• Erro ao calcular próximos horários\n"
    ...
    def enviar_relatorio(self, args):
        try:
            ...
            if not (1 <= mes <= 12):
                self.enviar_mensagem(
                    "❌ Mês inválido. Use um número entre 1 e 12.")
                return

            self.enviar_mensagem(f"🔄 Gerando relatório de {mes}/{ano}...")
            pdf_path = self.gerador_relatorios.gerar_relatorio_mensal(
                mes, ano, 'pdf')
    ...
    def mostrar_falhas(self, args):
        try:
            ...
            if dias <= 0 or dias > 90:
                self.enviar_mensagem(
                    "❌ Período inválido. Use entre 1 e 90 dias.")
                return
            ...
            if not falhas:
                self.enviar_mensagem(
                    f"✅ Nenhuma falha nos últimos {dias} dias")
                return
    ...
    def mostrar_horas(self, args):
        try:
            ...
            if not horas:
                self.enviar_mensagem(
                    f"ℹ️ Nenhuma hora registrada nos últimos {dias} dias")
                return
